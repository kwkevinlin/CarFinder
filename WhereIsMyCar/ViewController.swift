                ...
                if error == nil {
                    ...
                    if (self.carLat != self.coorLat || self.carLong != self.coorLong) {
                        ...
                        /*
                            ...
                            BUG:  1. After a series of new polylines generated by user movement (GPS), the
                                     last polyline won't be deleted because: if (carLat != coorLat..), the
                                     last step wont be updated, hence mapView.removeOverlay won't be ran.
                                     Positive caveate: it is unlikely (?) the original coordinates wont be
                                     stepped on exactly (ie. user sees car, turns off app / doesn't care
                                     about last few steps).
